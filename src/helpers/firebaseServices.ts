...
export const fetchCollectionData = async (collection: string) => {
    let data: any[] = []
    let querySnapshot = await firestore().collection(collection).orderBy('createdAt', 'desc').get()
...
export const saveDataInSubCollection = async (collection: string, doc: string, subCollection: string, jsonObject: any) => {
...
export const registerWithEmail = async (data: any, callback: Function) => {
    ...
    await auth().createUserWithEmailAndPassword(email, password)
        .then(async (userCredentials) => {
            ...
            const uploadedImage = await uploadImage(profileImage, `${STORAGE.PROFILE}/${userId}`)
...
export const loginWithEmail = async (email: string, password: string, callBack: Function) => {
    auth()
        ...
        .then(async data => {
            ...
            await updateDocument(FIREBASE_COLLECTION.USERS, data?.user?.uid, { fcmToken })
